...
exports.updateBags = functions.pubsub.schedule('every 24 hours from 01:00 to 01:30').onRun((() => {
        ...
        if (debtorGet.debtor) {
            if (debtorGet.debtor === "Feeds") {
                ...
                return batch.commit().then(() => {
                    return console.log("total thika farmers updated")
                })
                    .catch((err) => {
                        return console.error("error updating total thika farmers", err)
                    });
            }
        }
        return null;
