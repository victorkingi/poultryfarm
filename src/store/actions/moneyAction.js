        ...
        return firestore.runTransaction(function (transaction) {
            ...
            return transaction.get(currentDocRef).then(function (currentDoc) {
                if (currentDoc.exists) {
                    ...
                    firestore.collection("current").where("email", "==", receiver).get().then(function (querySnapshot) {
                        querySnapshot.forEach(function (doc) {
                            if (doc.exists) {
                                let receiverUID = doc.id;
                                const receiverDocRef = firestore.collection("current").doc(receiverUID);
                                let receiverName = doc.data().firstName + " " + doc.data().lastName;
                                const receiverNewBalance = doc.data().balance + amount;
                                ...
                                if (senderNewBalance < 0) {
                                    ...
                                    transaction.set(userLogRef, {
                                        event: 'sent money to ' + receiverName,
                                        amount: amount,
...
export const weClearedOurDebt = (details) => {
    return (dispatch, getState, {getFirebase, getFirestore}) => {
        ...
        return firestore.runTransaction(function (transaction) {
                        ...
                        return transaction.get(otherDebtDocRef).then(function (otherDebtDoc) {
                            if (currentDoc.exists) {
                                ...
                                if (final < 0 && user.email !== "jeffkarue@gmail.com") {
                                    ...
                                    transaction.set(userLogRef, {
                                        event: 'balance partly cleared of ' + details.debter,
                                        amount: balance,
                                        submittedBy: profile.firstName + ' ' + profile.lastName,
                                        submittedOn: firestore.FieldValue.serverTimestamp()
                                    });
            ...
            window.alert("Debt Cleared Successfully");
            window.location = '/';
        }).catch((err) => {
            const error = err.message || err;
                    ...
                    if (currentDoc.exists) {
                        if (oweJeffDoc.exists) {
                            ...
                            if (final < 0 && user.email !== "jeffkarue@gmail.com") {
                                ...
                                firestore.collection("current").where("fullName", "==", "Bank Account").get().then(function (querySnapshot) {
                                    querySnapshot.forEach(function (doc) {
                                        if (doc.exists) {
        ...
        return firestore.runTransaction(function (transaction) {
            ...
            return transaction.get(currentDocRef).then(function (currentDoc) {
                if (currentDoc.exists) {
                    ...
                    return Promise.reject("ERROR: Doc exists");
                }
            })
...
export const borrowerReturnsFunds = (details) => {
    return (dispatch, getState, {getFirebase, getFirestore}) => {
        ...
        return firestore.runTransaction(function (transaction) {
                ...
                return transaction.get(userLogRef).then(function (userLogDoc) {
                    if (currentDoc.exists) {
                        ...
                        firestore.collection("borrow").where("key", "==", details.key).get().then(function (querySnapshot) {
                            querySnapshot.forEach(function (doc) {
                                const borrowDocRef = firestore.collection("borrow").doc(doc.data().id);
                                const batch = firestore.batch();

                                batch.delete(borrowDocRef);
                                batch.commit().then(() => console.log("deleted borrow document"));
                            });
                        });
            ...
            window.alert("Money successfully received");
            window.location = '/';
        }).catch((err) => {
            const error = err.message || err;
